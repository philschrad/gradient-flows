...
# ╔═╡ a745ac84-2beb-11ec-18a2-7d9a9a23ad9a
begin
	using Plots
...
# ╔═╡ 8bdbbe0d-14df-4883-b532-6d654a1a37fd
md"""
Choose your Fourier coefficients!

a is just some parameter, c0 is the constant. 


cc is a list of the cosine coefficients, where the nth entry multiplies $$cos(n \theta)$$.

sc is a list of sine coefficients.

"""

# ╔═╡ 1fe5dcd8-bd7a-4ef8-a411-5fb7d9990bbb
begin
a=1/4
c0=-2-a
cc=[0,a/2,a/3,a/8]
...
# ╔═╡ e112ca71-8d1a-4628-898b-d17ff94b9f4d
md"""
Choose your timesteps!
"""

# ╔═╡ cde51d7d-baaf-4249-8321-10cdde20b3f0
begin
step_length=0.001
number_of_steps=50
end;

# ╔═╡ 8c490842-1022-4268-b7da-8386449df88a
begin
	ts=0:step_length:(number_of_steps*step_length)
	lambda(m,l)=(-1)^m*l^(2*m)*(l^2-1)^2
	summand(l,m,t,theta)=exp((-1)^(m+1)*lambda(m,l)*t)*(cc[l]*cos(l*theta)+sc[l]*sin(l*theta))
	dsummand(l,m,t,theta)=exp((-1)^(m+1)*lambda(m,l)*t)*((-l)*cc[l]*sin(l*theta)+l*sc[l]*cos(l*theta))
	evo(theta,t,m)=c0/2+sum([summand(l,m,t,theta) for l in 1:length(cc)])
	evo_theta(theta,t,m)=sum([dsummand(l,m,t,theta) for l in 1:length(cc)])
	
md"""
some functions are hiding here
"""
end

# ╔═╡ 5d07bb27-088f-4baf-b93a-84e3504b252c
md"""
Move the slider!
"""

# ╔═╡ eec11e56-4a93-4a24-bff3-306c37a2e4fc


# ╔═╡ a00272d4-3fb7-4756-bc8d-0d0af80d546f
@bind i Slider(1:length(ts))

# ╔═╡ 5039ee16-ea01-4c3c-92c9-d1fcf07d7ee0
md"""
Comparison of the flows with $$m=1$$ and $$m=2$$	

"""

# ╔═╡ b25f05c7-5113-47d4-99ec-c4e2308e7d48

begin

resolution=0.1
thetas=[i for i in 0:resolution:2*pi];
...
function gamma(u,u_theta,theta,t,m)
	-u(theta,t,m) *N(theta)+u_theta(theta,t,m)* T(theta)
end
...
function single_plot(step)
		Xx=append!(split_coords(step)[1],split_coords(step)[1][1])
   		Xy=append!(split_coords(step)[2],split_coords(step)[2][1])
...
function flow_series(stepsX,skip)
    ...
    Xx=append!(split_coords(stepsX[1])[1],split_coords(stepsX[1])[1][1])
    Xy=append!(split_coords(stepsX[1])[2],split_coords(stepsX[1])[2][1])
    plot(layout=(1,numplots),showaxis=false,link=:all)
    plot!(Xx,Xy,subplot=1,key=false,ratio=1,color="black",w=1,bg_inside=nothing, ticks=nothing)
    i=1
    while skip*i<=length(stepsX)
        Xx=append!(split_coords(stepsX[skip*i])[1],split_coords(stepsX[skip*i])[1][1])
        Xy=append!(split_coords(stepsX[skip*i])[2],split_coords(stepsX[skip*i])[2][1])
        plot!(Xx,Xy,subplot=i+1,key=false,color="black",w=1,ratio=1,bg_inside=nothing, ticks=nothing)
        #scatter!(Xx,Xy)
        i=i+1
    end
    plot!() 
...
md"""
more functions are hiding down here
"""
	
end

# ╔═╡ b2dce86a-20f8-42a2-b025-1893d5ff13a9
begin
	flow1=[[gamma(evo,evo_theta,theta,0,1) for theta in thetas]]
	for t in ts
		flow1=append!(flow1,[[gamma(evo,evo_theta,theta,t,1) for theta in thetas]])
	end
	flow2=[[gamma(evo,evo_theta,theta,0,2) for theta in thetas]]
	for t in ts
		flow2=append!(flow2,[[gamma(evo,evo_theta,theta,t,2) for theta in thetas]])
	end
end
	

# ╔═╡ 513a66da-3ff0-41a5-9874-d1cdb4571c1f
single_plot(flow1[i])

# ╔═╡ 38d570ff-5682-46ff-bda6-b64885de4103
begin
p1=flow_series(flow1,10)
p2=flow_series(flow2,10)
p=plot(p1,p2,layout=(2,1))

end

# ╔═╡ 3a72b235-29bd-4eb0-beed-e5b092b83d57
savefig(p,"./smoczyk.pdf")
